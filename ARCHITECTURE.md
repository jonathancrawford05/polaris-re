...
`analytics/portfolio.py::Portfolio` holds many `(InforceBlock, AssumptionSet, BaseTreaty)` deals and aggregates their `CashFlowResult`s into a `PortfolioResult` — aggregate NCF/IRR/PV, a per-deal breakdown, and concentration / HHI by cedant, product, and treaty type across three weight bases (ceded face, peak ceded NAR, PV premium). Calendar alignment places mixed-inception books on a common monthly grid; `run_scenarios()` applies the standard stress set across the whole portfolio. Each deal is projected on the reinsurer view by default. The book is editable after construction (ADR-178): `remove_deal` / `replace_deal` (position-preserving) / `clear_deals` mutate in place, `without_deal(*ids, name=...)` returns a filtered copy over the same frozen `Deal` objects — the what-if primitive, since `full.total_pv_profits - without_deal("B").run(h).total_pv_profits` is deal B's PV contribution under strict alignment — and `deal_ids` / `get_deal` / `len()` / `in` inspect it. `add_deal` and `replace_deal` share one `_build_deal` validation choke point, and an unknown deal id always raises `PolarisValidationError` rather than silently no-opping. Repeated runs can reuse per-deal projections via the **opt-in** result cache `Portfolio(..., cache=True)` (ADR-179), keyed `(deal_id, hurdle_rate)` on the instance: each of the four mutation verbs evicts the deals it invalidates (per deal, not book-wide, since `_run_deal` depends only on the deal and the hurdle rate), `run_with_capital` reuses what `run` cached, and a `without_deal` copy inherits the surviving entries by value — so a leave-one-out sweep over the whole book costs one projection per deal instead of one per deal per iteration. `_with_scenario` deliberately starts empty (a stressed deal keeps its id under scaled assumptions — the one key collision that would mask a stress). Caching is opt-in because a `Deal` holds mutable projection inputs; `clear_cache()` is the escape hatch and `cache_stats()` the timing-free observability surface. Cached and uncached runs are bit-identical.
