...
class Link:
    ...
    def __init__(self, name: str, linkinv: _VectorFn, mu_eta: _VectorFn) -> None:
        self.name = name
...
def binomial_logit() -> Family:
    ...
    logit_link = Link(
        "logit",
        linkinv=lambda eta: 1.0 / (1.0 + np.exp(-np.clip(eta, -700.0, 700.0))),
        mu_eta=lambda eta: (lambda p: np.clip(p * (1.0 - p), 1e-300, None))(
...
def binomial_cloglog() -> Family:
    ...
    **Not the canonical link for the binomial family** — unlike
    ...
    cloglog_link = Link("cloglog", linkinv=linkinv, mu_eta=mu_eta)
    return Family(
        "binomial",
        cloglog_link,
        _binomial_variance,
