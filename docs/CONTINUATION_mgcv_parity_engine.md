...
null space, exactly what `select=TRUE` is meant to produce) is reported
on the fit (`PolarisGAMFit.at_bound`/`.at_bound_blocks`) rather than
raised, with a new `strict=True` for the harness mode that still wants a
hard raise there. **Stage A**: `gam_select_penalty.null_space_penalty`
found and confirmed ONE basis-agnostic rule — eigendecompose the sum of a
term's own existing penalty block(s) at their natural, unscaled
...
47.36→16.96 figures (every case measured this session used a fixed,
