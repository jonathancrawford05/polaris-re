...
| | opening reserve `R_0`, ceded (50%) | peak, ceded |
|---|---:|---:|
| shipped code (`main`) | **$497,698.59** | $800,300.30 |
| the to-omega change | **$0.00** | $1,245,464 |

(The block's *gross* opening reserve is $995,397.19; the notebook cedes 50% on
coinsurance, and the ceded half is the right quantity here because it is what the
reinsurer's bond portfolio is sized against. At a 50% cession the retained and ceded
halves are equal, so "net" and "ceded" name the same $497,698.59 — spelled out because
"net" also names a *reserve basis* in this codebase, PR #191 [P2-4].)
...
a 10-years-in-force policy returns `V_0 = 0.00` on **both CRVM and GAAP**.

**Corrected in review (PR #191 [P1-1]) — VM-20 is not a fourth instance of this.** An
earlier revision of this ADR generalised the above to *"NET_PREMIUM's truncated recursion
was the only basis returning something non-zero for a seasoned policy"*. That is false.
Measured on shipped code, per policy, $500k face:

| years in force | NET_PREMIUM | CRVM | GAAP | **VM20** |
|---:|---:|---:|---:|---:|
| 0 | $5,067.67 | 0.00 | 0.00 | **$88,720.73** |
| 10 | $32,569.24 | 0.00 | 0.00 | **$242,857.93** |
| 15 | $19,718.95 | 0.00 | 0.00 | **$351,131.55** |
| 20 | $0.00 | 0.00 | 0.00 | **$497,901.99** |

VM-20 is `max(NPR, DR)`, and the **deterministic-reserve leg uses no
equivalence-principle premium at all** — it is a gross-premium reserve — so it returns a
non-zero seasoned reserve wherever the DR dominates, which on this fixture is
everywhere including at issue. The review found the same effect on its own fixture with
VM-20 at 0.00 for durations 0 and 10 and non-zero from 15 on; the crossover is
fixture-dependent, the mechanism is not.

So the correct statement is narrower and is the one that matters: **the three bases that
solve an equivalence-principle premium — NET_PREMIUM, CRVM, GAAP — all solve it at the
valuation date, and VM-20's NPR leg does too.** Only VM-20's DR leg escapes, and it
escapes by not having a valuation premium rather than by having a correct one.

This is not a bookkeeping correction. `PLAN_wl_valuation_premium.md` slice 1 carries an
acceptance criterion of `V_0 > 0` for a seasoned policy on every basis, and on VM-20 that
**would pass via DR dominance whether or not the fix reaches its NPR leg**. The plan's
Anchor 4 and its slice-1 test spec are amended accordingly.
...
- The **ALM duration-gap capability for whole life rests entirely on that defect.** Its
  liability present value *is* the opening reserve, and on the REST `SEASONED_POLICY`
  fixture ($1,000,000 face, 5 years in force) that liability is already negligible on
  shipped `main`. **Stated with its parameterisation, because the figure moves with the
  treaty and the measurement yield** (PR #191 [P1-2] — an earlier revision quoted one
  number bare, and the review reproduced a different one from different settings):

  | product | treaty | `alm_valuation_yield` | liability PV | liability `D_mod` |
  |---|---|---|---:|---:|
  | WHOLE_LIFE | YRT 90% | default (0.06) | **$25.48** | 11.824 |
  | WHOLE_LIFE | YRT 90% | 0.08 | **$20.34** | 10.701 |
  | WHOLE_LIFE | Coinsurance 50% | default (0.06) | **$12.74** | 11.824 |
  | WHOLE_LIFE | Coinsurance 50% | 0.08 | **$10.17** | 10.701 |
  | TERM | YRT 90% | default (0.06) | $21.71 | 0.111 |

  All at `discount_rate=0.06`. Every cell is ~$10–25 of liability present value against
  $1,000,000 of face: the capability does not work on whole life, and the treaty and
  yield only change which negligible number you get. The notebook's larger block hides
  this behind a plausible-looking $497,698.59.

  **One further correction:** an earlier revision said this fixture also showed a 0/0
  modified duration of ~1e15 on shipped code. It does not — the shipped duration is a
  perfectly ordinary 11.824. The ~1e15 blow-up appeared only **after** the withdrawn
  change drove the liability to exactly $0.00, and attributing it to `main` overstated
  the case.
